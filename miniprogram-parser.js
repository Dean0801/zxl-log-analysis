  ...
  jsonData.forEach((record, index) => {
    try {
      ...
      const properties = {
        ...
        // 书籍信息
        bookId:
          userAttributes.bookId ||
          analysisData.book_id ||
          lineData.args?.bookId,
        ...
        failReason:
          analysisData.fail_reason ||
          analysisData.failReason ||
          lineData.failReason ||
          lineData.fail_reason,
      ...
      if (properties.failReason) {
        ...
        // 解析 [response] 部分
        const responseMatch = reasonStr.match(
          /\[response\]:\s*\n?\s*(\{[\s\S]*?\})(?=\n?\s*\[|$)/
        );
        if (responseMatch) {
          try {
        ...
        // 解析 [error] 部分
        const errorMatch = reasonStr.match(
          /\[error\]:\s*\n?\s*(.+?)(?=\n?\s*\[|$)/s
        );
        if (errorMatch) {
      ...
      allData.push({
        ...
        rawData: lineData,
        hasTooltip:
          MINIPROGRAM_EVENT_MAP[eventName]?.hasTooltip ||
...
function generateTreeStructure(
  ...
  const treeId = `tree-${sectionTitle
    .toLowerCase()
  ...
  if (copyData) {
    html += `<button class="tree-section-copy" data-copy="${encodeURIComponent(
      JSON.stringify(data, null, 2)
    )}" onclick="copyData(this)">复制</button>`;
  }
...
function generateTreeNodes(data, depth = 0, path = "") {
  ...
  keys.forEach((key) => {
    ...
    const nodeId = `node-${fullPath.replace(
      /[^a-zA-Z0-9]/g,
      "-"
...
export function getMiniprogramEventDetail(item) {
    ...
    if (props.stack) {
      ...
      details.push(`<div class="pay-detail-item">
				...
				<span class="pay-detail-value">
					...
					<button class="copy-btn" data-copy="${encodeURIComponent(
            props.stack
          )}" onclick="copyData(this)">复制</button>
				</span>
			</div>`);
  ...
  if (deviceManufacturer || deviceModel) {
    deviceInfoDetails.push(`<div class="pay-detail-item">
			...
			<span class="pay-detail-value">${deviceManufacturer || "Unknown"} ${
      deviceModel || ""
    }</span>
		</div>`);
    hasDeviceInfo = true;
  ...
  if (hasDeviceInfo) {
    details.push(
      '<div class="device-info-header" style="margin: 12px 0 8px 0; padding: 4px 8px; background: rgba(33, 150, 243, 0.1); border-radius: 4px; font-size: 12px; font-weight: bold; color: #2196f3;">📱 设备信息</div>'
    );
  ...
  if (item.event === "user_login") {
    ...
    if (launchQuery && Object.keys(launchQuery || {}).length) {
      const queryJson =
        typeof launchQuery === "string"
          ? launchQuery
          : JSON.stringify(launchQuery, null, 2);
      details.push(`<div class="pay-detail-item">
				<span class="pay-detail-icon">🔎</span>
  ...
  if (props.miniprogramName) {
    details.push(
      '<div class="device-info-header" style="margin: 12px 0 8px 0; padding: 4px 8px; background: rgba(33, 150, 243, 0.1); border-radius: 4px; font-size: 12px; font-weight: bold; color: #2196f3;">📱 小程序信息</div>'
    );
    details.push(`<div class="pay-detail-item">
			<span class="pay-detail-icon">📱</span>
  ...
  if (props.bookName) {
    details.push(
      '<div class="device-info-header" style="margin: 12px 0 8px 0; padding: 4px 8px; background: rgba(33, 150, 243, 0.1); border-radius: 4px; font-size: 12px; font-weight: bold; color: #2196f3;">📚 书籍信息</div>'
    );
    details.push(`<div class="pay-detail-item">
			<span class="pay-detail-icon">📚</span>
  ...
  if (props.adType) {
    details.push(
      '<div class="device-info-header" style="margin: 12px 0 8px 0; padding: 4px 8px; background: rgba(33, 150, 243, 0.1); border-radius: 4px; font-size: 12px; font-weight: bold; color: #2196f3;">📺 广告信息</div>'
    );
    details.push(`<div class="pay-detail-item">
			<span class="pay-detail-icon">📺</span>
  ...
  if (props.isSuccess !== undefined) {
    ...
    details.push(
      '<div class="device-info-header" style="margin: 12px 0 8px 0; padding: 4px 8px; background: rgba(33, 150, 243, 0.1); border-radius: 4px; font-size: 12px; font-weight: bold; color: #2196f3;">⚙️ 状态信息</div>'
    );
    details.push(`<div class="pay-detail-item" style="color: ${
      success ? "#4caf50" : "#f44336"
    }">
			<span class="pay-detail-icon">${success ? "✅" : "❌"}</span>
			<span class="pay-detail-label">状态:</span>
			<span class="pay-detail-value" style="font-weight: bold">${
        success ? "成功" : "失败"
      }</span>
		</div>`);
  }
  ...
  if (failReason) {
    details.push(
      '<div class="device-info-header" style="margin: 12px 0 8px 0; padding: 4px 8px; background: rgba(33, 150, 243, 0.1); border-radius: 4px; font-size: 12px; font-weight: bold; color: red;"> 🔍 失败详情</div>'
    );

    const reasonStr = String(failReason);

    // 解析 [method] 部分 - 处理换行符
    const methodMatch = reasonStr.match(
      /\[method\]:\s*\n?\s*(\{[\s\S]*?\})(?=\n?\s*\[|$)/
    );
    if (methodMatch) {
      try {
  ...
  // [method] 部分
  if (methodData) {
    const treeHtml = generateTreeStructure(
      methodData,
      "method",
      "📨",
      methodData
